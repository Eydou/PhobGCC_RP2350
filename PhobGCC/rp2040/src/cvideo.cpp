...
static double cvideo_pio_clkdiv() {
	return (double)clock_get_hz(clk_sys) / VIDEO_horizontal_freq / PIO_clkdot / HORIZ_dots;
}
...
int videoOut(const uint8_t pin_base,
	...
	cvideo_initialise_pio(pio, state_machine, offset, pin_base, 4, cvideo_pio_clkdiv()); // Initialise the PIO (function in cvideo.pio)
