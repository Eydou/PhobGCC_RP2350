...
int main() {
	//The Joybus PIO divider is now derived from the active system clock,
	//so the comms timing stays correct on both RP2040 and RP2350.
	...
	if(_hardware.Z) { //hold Z on powerup for PhobVision
		_videoOut = true;
		set_sys_clock_khz(1000*250, true);//overclock to 250 MHz, to alleviate performance issues
	}
