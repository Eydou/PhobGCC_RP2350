...
constexpr float JOYBUS_PIO_CLOCK_HZ = 25'000'000.0f;
...
/* PIOs are separate state machines for handling IOs with high timing precision. You load a program into them and they do their stuff on their own with deterministic timing,
   ...
   The PIO program expects a 25MHz state machine clock. We derive the divider from the actual system clock so the same timings work on RP2040 and RP2350.
...
void __time_critical_func(enterMode)(const int dataPin,
    ...
    sm_config_set_clkdiv(&config, (float)clock_get_hz(clk_sys) / JOYBUS_PIO_CLOCK_HZ);
